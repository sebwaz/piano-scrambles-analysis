...
while true
    ...
    % Constrain sum(F_1) = length(F_1)
    idx = (size(dp, 1) + 1):(size(dp, 1) + size(dp, 2));
    tmp(idx) = size(dp, 2) * tmp(idx) / sum(tmp(idx));
    
    % Constrain sum(F_2) = length(F_2)
    % Note to self: colon operator is last in order of operations, so needs to be parenthesized
    idx = ((size(dp, 1) + 1):(size(dp, 1) + size(dp, 2))) + size(dp, 2) + size(dp, 1);
    tmp(idx) = size(dp, 2) * tmp(idx) / sum(tmp(idx));
...
stdev = params(end);
dp_hat = R_s_1' * F_t_1 + R_s_2' * F_t_2;
...
n_params = 2 * size(dp, 1) + 2 * size(dp, 2) + 1;
