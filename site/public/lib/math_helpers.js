...
// Generate a random int from 1 to max-1
...
// Create a 1 x n vector that represents a white noise signal
// Specifically use N(mean = 0, var = 1/3.0902) because norminv(0.999) = 3.0902
// Which means we expect one clipped sample every 500 samples (both negative or positive clipped).
// Manually set clipped values to 1 or -1
function white_noise(n) {
  var result = new Array(n);
  for (let k = 0; k < n; k++) {
    // Based on Box-Mueller transformation, using optimization by Vortico on SO: https://stackoverflow.com/questions/25582882/javascript-math-random-normal-distribution-gaussian-bell-curve
    result[k] = Math.sqrt(-2 * Math.log(1 - Math.random())) * Math.cos(2 * Math.PI * Math.random());
  }
  
  // Set variance equal to 1/3.0902
  result = sv_prod(1 / Math.sqrt(3.0902), result);
